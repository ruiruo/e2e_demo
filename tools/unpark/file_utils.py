...
from unpark.constants import GREEN, RESET
import copy
import glob
...
def parse_tfrecord(record_path, fea_cfg: dict = {}, label_match_mode: str = False):
    ...
    def parse_func(record):
        try:
            ...
            # features = {
            #     fea_name: tf.io.VarLenFeature(dtype=getattr(tf, val["type"]))
            #     for fea_name, val in fea_cfg.items()
            # }
...
# def load_data_by_type(data_dir, input_type: str = "cost_data", prefix: str = ""):

#     if input_type == "cost_data":
#         with open(f"{data_dir}/{prefix}_train_input.pkl", "rb") as f:
#             train_data = pickle.load(f)
#         with open(f"{data_dir}/{prefix}_test_input.pkl", "rb") as f:
#             test_data = pickle.load(f)

#     elif input_type == "traj_data":
#         with open(f"{data_dir}/{prefix}_train_traj.pkl", "rb") as f:
#             train_data = pickle.load(f)
#         with open(f"{data_dir}/{prefix}_test_traj.pkl", "rb") as f:
#             test_data = pickle.load(f)
