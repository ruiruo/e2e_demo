...
class Bag2E2E:
    def __init__(self, bag_file_path, output_folder, config_file_path, start_time=0.0, duration_time=-1,
                 pose_format="euler_angle", sample_rate=1, image_channel_list=["left", "right", "front", "back"]):
    ...
    def update_save_state(self, length_threshold=15):
        if not self.record_start:
            if self.driving_state == self.record_state:
                ...
                self.segment_path = os.path.join(self.output_folder_train,
                                                 "{}".format(str(int(self.current_time.to_sec()))))
                self.name_cnt = 0
    ...
    def transfer(self):
        save_trigger = False
        for topic, msg, current_time in tqdm.tqdm(
                self.bag.read_messages(topics=self.check_topics, start_time=Time.from_sec(self.start_time))):
            ...
            if topic in self.camera_topic:
                ...
                if current_channel not in self.image_channel_list:
                    continue
                self.image_bank[current_channel] = self.parser_compressed_image_msg(msg, undsitort=True,
...
def main(bag_file_path, output_folder_path, config_file_path, start_time, duration_time, sampling_rate, camera_config):
    transform_obj = Bag2E2E(bag_file_path, output_folder_path, config_file_path, start_time, duration_time,
